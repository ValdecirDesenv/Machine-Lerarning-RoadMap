...
def main():
    ...
    # Merge the DataFrames on df1's column 'account_number' and df2's column 'account_number' using an inner join
    trans_bonus_up = pd.merge(trans_bonus_up, crm_filtered, on='account_number', how='inner')

    # Merge the DataFrames again to add the 'account_level_fee' column using a left join
    trans_bonus_up = pd.merge(trans_bonus_up, crm_filtered[['account_number', 'account_level_fee']], on='account_number', how='left')

    # Drop the redundant 'account_number' column from the merged DataFrame
    trans_bonus_up = trans_bonus_up.drop(columns=['account_number'])
